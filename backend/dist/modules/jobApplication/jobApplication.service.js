...
exports.jobApplicationService = {
     ...
     */
    async applyToJob(jobId, workerId) {
    ...
    async getJobApplicants(jobId, employerId) {
        ...
        if (job.employerId !== employerId) {
            throw new response_1.ApiError(constants_1.HTTP_STATUS.FORBIDDEN, "You can only view applicants for your own jobs");
        }
        return prisma_1.prisma.jobApplication.findMany({
            where: { jobId },
            include: {
    ...
    async selectWorkersForJob(jobId, employerId, workerIds) {
        return await prisma_1.prisma.$transaction(async (tx) => {
            ...
            if (job.employerId !== employerId) {
                throw new response_1.ApiError(constants_1.HTTP_STATUS.FORBIDDEN, "You can only select workers for your own jobs");
            }
            if (job.status !== "OPEN") {
                throw new response_1.ApiError(constants_1.HTTP_STATUS.BAD_REQUEST, "Job must be in OPEN status to select workers");
            }
