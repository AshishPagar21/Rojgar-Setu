...
exports.jobService = {
    ...
    async updateJob(jobId, employerId, payload) {
        ...
        const now = new Date();
        if (now >= jobStart) {
            throw new response_1.ApiError(constants_1.HTTP_STATUS.BAD_REQUEST, "Cannot edit a job after it has started or during its scheduled time");
        }
        const hoursRemaining = (jobStart.getTime() - now.getTime()) / (1000 * 60 * 60);
        if (hoursRemaining < 10) {
            throw new response_1.ApiError(constants_1.HTTP_STATUS.BAD_REQUEST, "Cannot edit job within 10 hours of job start time");
        }
        if (!payload.latitude || !payload.longitude) {
            throw new response_1.ApiError(constants_1.HTTP_STATUS.BAD_REQUEST, "Location coordinates (latitude and longitude) are required");
        }
