...
export const jobApplicationService = {
  ...
  async applyToJob(jobId: number, workerId: number) {
    ...
    const selectedApplicationsOnSameDay = await prisma.jobApplication.findMany({
      where: {
        workerId,
        status: "SELECTED",
        job: {
    ...
    if (activeSelectedApplications.length > 0) {
      ...
      throw new ApiError(
        HTTP_STATUS.BAD_REQUEST,
        `You cannot apply to this job because you are already selected for another job ("${activeApp.job.title}") on this day.`,
      );
    }
